...
def SubmitOrder(name:str, quantity:int):
    message = "Add {0} Printout for {1}".format(str(quantity), name)
    response = messagebox.askquestion(title="SUBMIT",message=message)
    if response =='yes':
        insert_order(name,quantity,"Printout")
        message= "Successfully Added {0} Printout for {1}".format(str(quantity), name)
...
def CreatePrintoutFrame(parent, page_change):
    ParentFrame = Frame(parent)
    MainTitle = Label(ParentFrame, text="Add Prints", pady=20, font=("Arial", 25, "bold"), bg="White")
    ...
    SubmitButton = Button(EnterDataFrame, text="Submit", font="Arial 20 bold", bg="white", command= lambda : SubmitOrder(str(ClientSelection.get()),int(QuantityEntry.get())))
    SubmitButton.grid(row=5, columnspan=2)

    add_separator(EnterDataFrame, row=6,height=5)

    CorrectionButton = Button(EnterDataFrame, text="Make\nCorrection", font="Arial 20 bold", bg="white", command=lambda : page_change("CorrectPrintout"))
